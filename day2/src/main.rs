...
impl Entry {
    ...
    fn is_valid2(&self) -> bool {
        let low_char = self.password.chars().nth(self.low-1).unwrap();
        let high_char = self.password.chars().nth(self.high-1).unwrap();
        if low_char == self.target || high_char == self.target {
            return low_char != high_char
        }
        false
    }
}
